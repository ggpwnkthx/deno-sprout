 ...
 *
 * The generated file exports a `mount(Component, props, el)` function that:
 * 1. Calls `Component(props)` to produce a Hono JSX node.
 * 2. Serialises it to an HTML string via `renderToString` from
 *    `stringify.ts` (browser-compatible, no Node.js required).
 * 3. Sets `el.innerHTML` once.
 * 4. Returns a no-op dispose function (reserved for future cleanup).
 *
...
export async function buildMount(outdir: string): Promise<void> {
  // Read the actual mount.ts source from the islands package so the build
  // output is always in sync with the canonical source. This eliminates
  // template drift where an embedded copy diverges from the real file.
  const mountSource = await Deno.readTextFile(
    join(dirname(fromFileUrl(import.meta.url)), "../islands/lib/mount.ts"),
  );
  ...
  // Smoke-test: verify the template produced the expected import and error
  // handling. esbuild minifies class names (e.g. RenderError → d,
  // IslandErrorEvent → a) but preserves string literals used in imports and
  // keeps the `instanceof <name>` spacing pattern, so we check for:
  //   1. "stringify"       — the module is imported (string literal preserved)
  //   2. /instanceof\s+\w/ — actual instanceof usage survives minification
  if (!result.code.includes("stringify")) {
    throw new Error(
      "[buildMount] Template did not import from stringify.ts — template may have drifted",
    );
  }
  if (!/instanceof\s+\w/.test(result.code)) {
    throw new Error(
      "[buildMount] Template is missing instanceof checks — template may have drifted",
    );
  }
